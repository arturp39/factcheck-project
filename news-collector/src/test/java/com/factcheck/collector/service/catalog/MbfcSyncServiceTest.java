...
class MbfcSyncServiceTest {
    ...
    void syncAndMap_savesSourcesAndMapsPublishers() {
        ...
        Publisher byDomainOne = Publisher.builder()
                .id(1L)
                .name("LiveLeak")
        ...
        ArgumentCaptor<List<MbfcSource>> sourceCaptor =
                (ArgumentCaptor<List<MbfcSource>>) (ArgumentCaptor<?>) ArgumentCaptor.forClass(List.class);
        verify(mbfcSourceRepository).saveAll(sourceCaptor.capture());
        List<MbfcSource> savedSources = sourceCaptor.getValue();
        assertThat(savedSources).hasSize(2);
        assertThat(savedSources)
                .anyMatch(source -> source.getMbfcSourceId().equals(104914L))
                .anyMatch(source -> source.getMbfcSourceId().equals(107142L));
